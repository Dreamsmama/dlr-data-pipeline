...
mkdir -p "$INSTALL_ROOT" "$SHARED_DIR" "$DATA_DIR" "$LOG_DIR" "$INCOMING_DIR" "$RELEASES_DIR"
...
prepare_archive_release() {
  local archive_listing archive_path release_dir temporary_dir
  archive_path="$(readlink -f "$SOURCE_ARCHIVE")"
  case "$archive_path" in
    "$INCOMING_DIR"/*) ;;
    *)
      log "部署制品必须位于 ${INCOMING_DIR}"
      return 1
      ;;
  esac
  if [[ ! -f "$archive_path" ]]; then
    log "部署制品不存在：${archive_path}"
    return 1
  fi
  if [[ ! "$SOURCE_VERSION" =~ ^[0-9a-f]{7,40}$ ]]; then
    log "DEPLOY_SOURCE_VERSION 必须是 Git 提交 ID"
    return 1
  fi
  ...
  if [[ ! -d "$release_dir" ]]; then
    ...
    if [[ ! -f "${temporary_dir}/deploy.sh" || ! -f "${temporary_dir}/docker-compose.yml" ]]; then
      log "部署制品缺少 deploy.sh 或 docker-compose.yml"
      return 1
    fi
    mv "$temporary_dir" "$release_dir"
  fi
  REPOSITORY_DIR="$release_dir"
}
...
if ! command -v git >/dev/null || ! command -v curl >/dev/null || ! command -v flock >/dev/null || ! command -v tar >/dev/null; then
  log "安装 Git、curl、tar 和部署锁依赖"
  apt-get update
  apt-get install -y git curl ca-certificates tar util-linux
fi
...
if [[ -n "$SOURCE_ARCHIVE" ]]; then
  log "使用已上传制品 ${SOURCE_VERSION}，跳过服务器 GitHub 拉取"
  prepare_archive_release
else
...
if [[ -L "${SHARED_DIR}/current-release" ]]; then
  OLD_COMPOSE_FILE="$(readlink -f "${SHARED_DIR}/current-release/docker-compose.yml" 2>/dev/null || true)"
fi
...
export PUBLIC_PORT
if [[ -n "$SOURCE_ARCHIVE" ]]; then
  DEPLOY_IMAGE_TAG="${SOURCE_VERSION:0:12}"
else
...
ln -sfn "$REPOSITORY_DIR" "${SHARED_DIR}/current-release"
SWITCH_STARTED=false
